...
const ManageStudent = () => {
    ...
    const fetchEmployees = async () => {
      try {
        const response = await axios.get(
          "http://localhost:4000/student/getAllStudent"
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:4000/student/deleteStudent/${id}`); // Your backend endpoint for deletion
  ...
  const handleUpdateEmployee = async () => {
    try {
      await axios.put(
        `http://localhost:4000/student/updateStudent/${currentEmployee.id}`,
        currentEmployee
      ); // Update endpoint
